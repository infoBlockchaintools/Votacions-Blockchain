...
<h2 style="text-align:center">RESTABLIR EL REGISTRE - Insertar dades</h2>
	<form class="login-form" name="form" id="form" method="post" style="width:95%" action="restablir.php" style="font-size:12px">
		...
		<div style="width:22%;float:left;padding:15px;">				
			Contrasenya
			<input type="text" id="sPass" name="sPass" class="form-control" placeholder="" required autofocus maxlength="150">
		</div>
		<div style="clear:both"></div>
		<br/>			<br/>	 
		<button  class="boto1" id="boto1">Restablir accés</button>
	</form>			
<?php 
}
else
{
?>
<h2 style="text-align:center">RESTABLIR REGISTRE - Resultat</h2>
	<?php
	...
	$sql = "SELECT id,idRegistre from 1_censGlobal WHERE idControl = '" . $campATrobar . "' and idEntitat = " . $entitat;
	$results = $conn->Select($sql,$link); 
	...
	if ($row["id"]!="") {
		...
		$sql = "DELETE FROM 4_errors WHERE sEquivocat = '" . $hashErrors ."'";
		$conn->Select($sql,$link); 		
		echo "<h2>Usuari RESTABLERT. Errors de bloqueix també inicialitzats</h2>";
	}	
	else
	{
